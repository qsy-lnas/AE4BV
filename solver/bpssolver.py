...
# problem_data_path = "data/Data_Problem_0925"
# train_data_path = 'data/Data_Test_0913'
...
def Solver(t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, rnd):
    # env = gp.Env(logfilename = f"result/log/gurobi_{rnd}.log")
    env = gp.Env()
    ...
    #initial constraints
    #initial amount
    model.addConstrs((state_amount[s, 'e0'] == initial_amount[s] for s in states), name = 'InitialAmount')
    ...
    #initial processing
    model.addConstrs((processing[i, j, n]  <= state_amount[s, n] for i in tasks for j in units if suitability.get((j, i), 0) == 1 for s in states if proportions_consumed[s, i] < 0 for n in events), name = 'ProcessingConstraints')
    #storage
    ...
    #time horizon
    model.addConstrs((t_finish[i, j, n] <= H for i in tasks for j in units if suitability.get((j,i),0) == 1 for n in events), name = 'Time_Horizon_finish')
    model.addConstrs((t_start[i, j, n] <= H for i in tasks for j in units if suitability.get((j,i),0) == 1 for n in events), name = 'Time_Horizon_start')
    ...
    model.optimize()
    # model.write(f"{problem_data_path}/{(rnd):0>4}.lp")

    #save the result
    # model.write(f'{train_data_path}/{(rnd):0>4}.sol')
    ...
    # y = [1, 2, 3, 4, 5, 6, 7, 8]
    # plt.yticks(y, labels)
    # plt.title('Short-term Scheduling of 48H')
...
def solve_else(solver, mpspath, logflag = 0, saveflag = 0, mipgap = 0.0, thread = 1):
    ...
    if solver == "copt":
        ...
        model.setLogFile(f"result/log/{solver}_{mpspath.split('.')[0].split('/')[-1]}.log")
        # model.setParam(COPT.Param.logfile, f"result/log/{solver}_{mpspath.split('.')[0].split('/')[-1]}.log")
...
for i in range(1000, 1200):
    ...
    """ other solver """
    # o, t = solve_else("copt", f'data/Data_Problem_5pt/{(i):0>4}.lp')
    time_record.append(t)
    opt_record.append(o)

    with open(f'result/value_origin_{param}_copt.csv', mode='a', newline='') as file:
           writer = csv.writer(file)
           writer.writerow(opt_record)
    with open(f'result/time_origin_{param}_gurobi.csv', mode='a', newline='') as file:
           writer = csv.writer(file)
